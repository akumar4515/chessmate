...
export default function ExitConfirmDialog({ visible, title = 'Exit ChessMate', message = 'Are you sure you want to exit?', cancelText = 'Cancel', exitText = 'Exit', onCancel, onExit }) {
  ...
  return (
    <Modal animationType="none" transparent visible={visible} onRequestClose={()=>{playClick(),onCancel()}} hardwareAccelerated>
      <Animated.View style={[styles.overlay, { opacity }]}>
        <Animated.View style={[styles.card, { transform: [{ scale }] }]}>
          <Text style={styles.title}>{title}</Text>
          <Text style={styles.message}>{message}</Text>
          <View style={styles.row}>
            <TouchableOpacity activeOpacity={0.85} style={{ flex: 1, marginRight: 8 }} onPress={()=>{playClick(),onCancel()}}>
              <View style={styles.ghostBtn}><Text style={styles.ghostText}>{cancelText}</Text></View>
            </TouchableOpacity>
            <TouchableOpacity activeOpacity={0.9} style={{ flex: 1 }} onPress={()=>{playClick(),onExit()}}>
